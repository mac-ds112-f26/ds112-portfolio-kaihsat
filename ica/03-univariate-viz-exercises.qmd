...
```{r}
ggplot(data = hikes, aes(x = rating)) +
...
the amount of hikes at each level
...
labeling the x value as rating
...
# We added an outline color to the bars
```
...
# We turned the background color from grey to white
```
...
when we want to add another line of code to our plot 

-   We added the bars using `geom_bar()`? Why "geom"?

I think geom stands for geometry so it represents the shapes of the bar
...
color is the outline and fill isthe middle of the bar
...
We dont know what the easy, moderate and hard ratings mean
...
#there are too many observations and they all have different elevations, so the bars are too thin and the y axis isnt informative
```
