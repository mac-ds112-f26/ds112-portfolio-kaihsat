...
does this mean there are 75% support for republicans across all counties
...
# How does this differ from the frame for our histogram of repub_pct_20 alone? theres nothing
...
# What happens if you change the shape to another number? each number is a sifferent shape
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
  geom_point(shape = 3)
```
...
# NOTE: Try to anticipate if "color" or "fill" will be useful here. Then try both. it was color
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
...
strong relationship between 2016 and 2020, positive, texas, utah, and idaho 
...
```{r}
#it doesnt ansewr many questions because its very hard to read
...
In red states, 75% of counties vote republican with many outliers that vote blue
in blue states 55% of counties vote republican 
...
# the colors on the graph dont match the colors of the parties
# The right side of the purple and blue visualizations is hidden
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density()
```
...
# Makes the visualizations more or less opaque
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
...
# because you cant see most of the data
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
...
I like looking at the shape of the data
-   What is one con of density plots relative to boxplots?
You dont get data about the averages and outliers
